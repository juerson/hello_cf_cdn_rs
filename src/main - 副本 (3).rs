    ...
    match TcpStream::connect_timeout(&server_addr, connect_timeout_duration) {
        Ok(mut stream) => {
            ...
            if let Err(_err) = stream.read_exact(&mut response) {
                log::info!("{}:{}, false, 服务端无响应！", address, port);
                return (address.to_string(), port, false, format!("服务端无响应！"));
            }
        ...
        Err(_err) => {
            log::info!("{}:{}, false, 连接超时！", address, port);
            (address.to_string(), port, false, format!("连接超时！"))
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    */
    ...
    println!("--------------------------------------------------------------------------");
    println!("正在发送数据包过去测试每个端口{:?}的延迟状况....", ports);
    println!("--------------------------------------------------------------------------");
    ...
    if !true_results.is_empty() {
        ...
        if flag {
            println!("-------------------------------------------------------------------------");
            println!("结果已经写入文件: {}", final_file_name);
        }
    }
